...
func _on_area_3d_input_event(camera, event, position, normal, shape_idx):
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed == true:
			...
			Global.answerkeeper = Global.answerkeeper.append_array(Global.answerstate2)
			print(Global.answerkeeper)

			var correct = true
			for n in Global.answerstate2.size():
				if not Global.answerstate2[n] == Global.answersq2[n]:
					correct = false
			if correct == true:
				Global.q1 = true
				$Jatka.show()
